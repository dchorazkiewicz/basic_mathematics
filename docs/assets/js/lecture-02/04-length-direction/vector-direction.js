(() => {
  ...
  board.create('text', [() => P.X() / 2, -.45, () => `v₁ = ${P.X().toFixed(1)}`], { color: '#2f6f9f', fontSize: 17, ...fixed });
  board.create('text', [() => P.X() + .25, () => P.Y() / 2, () => `v₂ = ${P.Y().toFixed(1)}`], { color: '#b1782b', fontSize: 17, ...fixed });
  board.create('text', [() => P.X() / 2 - .5, () => P.Y() / 2 + .45, () => `‖v‖ = ${Math.hypot(P.X(), P.Y()).toFixed(2)}`], { color: '#7a3f73', fontSize: 17, cssClass: 'distance-hypotenuse-label', ...fixed });
  const update = () => {
