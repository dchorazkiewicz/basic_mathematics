<section id="parametric-curves" class="section">
  <h2>Curves described by one parameter</h2>
  <p>A parametric curve uses one number \(t\) to produce both coordinates at once:</p>
  <div class="math-block">\[t\longmapsto (x(t),y(t)).\]</div>
  <p>As \(t\) changes, the point moves. The complete path traced by that moving point is the curve. A circle is therefore still one-dimensional: one parameter is enough to move around it.</p>
