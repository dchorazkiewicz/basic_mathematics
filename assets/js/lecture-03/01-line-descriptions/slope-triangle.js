(() => {
  ...
  const P0 = board.create('point', [1, 1], {
    name: 'P₀', size: 4, fillColor: '#18212b', strokeColor: '#18212b',
    label: { offset: [-28, -24], fontSize: 18, cssStyle: labelStyle }, ...fixed
  });

  const V = board.create('point', [5, 3.5], {
    name: 'P₀⊕v', size: 6, fillColor: '#2f6f9f', strokeColor: '#2f6f9f',
    label: { offset: [10, 12], fontSize: 18, cssStyle: labelStyle },
    snapToGrid: true, snapSizeX: 0.5, snapSizeY: 0.5
  });

  const constrain = () => {
    const x = Math.max(P0.X() + 0.5, V.X());
    const y = Math.max(-3, Math.min(4.5, V.Y()));
    V.setPosition(JXG.COORDS_BY_USER, [x, y]);
  };
  V.on('drag', constrain);

  const H = board.create('point', [() => V.X(), () => P0.Y()], {
  ...
  board.create('polygon', [
    H,
    [() => V.X() - 0.25, () => P0.Y()],
    [() => V.X() - 0.25, () => P0.Y() + Math.sign(V.Y() - P0.Y() || 1) * 0.25],
    [() => V.X(), () => P0.Y() + Math.sign(V.Y() - P0.Y() || 1) * 0.25]
  ], {
  ...
  const horizontalReference = board.create('point', [() => P0.X() + 1, () => P0.Y()], {
    visible: false, ...fixed
  });
  board.create('angle', [horizontalReference, P0, V], {
