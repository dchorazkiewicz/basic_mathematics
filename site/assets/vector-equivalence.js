(() => {
  ...
  const P = board.create('point', [-2.5, -1.5], { name: 'P', size: 5, fillColor: '#3f735f', strokeColor: '#3f735f', snapToGrid: true, snapSizeX: .5, snapSizeY: .5 });
  const P2 = board.create('point', [-.5, 2], { name: "P'", size: 5, fillColor: '#b1782b', strokeColor: '#b1782b', snapToGrid: true, snapSizeX: .5, snapSizeY: .5 });
  const d = [3, 2];
  const Q = board.create('point', [() => P.X() + d[0], () => P.Y() + d[1]], { name: 'Q', visible: true, size: 4, fillColor: '#3f735f', strokeColor: '#3f735f', ...fixed });
  const Q2 = board.create('point', [() => P2.X() + d[0], () => P2.Y() + d[1]], { name: "Q'", visible: true, size: 4, fillColor: '#b1782b', strokeColor: '#b1782b', ...fixed });
  ...
  board.create('text', [-5, 5, 'Both arrows have coordinate change [3,2].'], { color: '#66717d', fontSize: 17, ...fixed });
